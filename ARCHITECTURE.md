...
5. **Prompt Engineering (Server):** The Next.js API route validates the input and constructs a strict prompt for the Gemini API, requesting a structured JSON response.
6. **LLM Inference (Server → Gemini):** The server securely calls the Gemini API using the `GEMINI_API_KEY`.
7. **Response Parsing (Server):** The server strips any markdown code blocks from the Gemini response, strictly parses it as JSON, validates it against the `EmailContent` interface, and sends it back to the client.
8. **UI Rendering (Client):** `App.tsx` receives the parsed JSON, updates the `currentEmail` state, adds it to the `history` state, and sets `isLoading = false`. The `<GeneratedEmailPanel />` and `<HistoryPanel />` instantly re-render to display the new data.
